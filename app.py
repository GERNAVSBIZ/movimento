...
try:
    ...
    if os.path.exists(credentials_path):
        ...
        # Este aviso é crucial para depuração no Render
    ...
    # Previne a reinicialização que causa erros em ambientes de deploy
...
def get_files():
    ...
    print("[LOG get-files] Rota /api/get-files acessada.")
    try:
        # ABORDAGEM ROBUSTA: Busca os documentos sem ordenação para evitar erros de índice
        files_ref = db.collection(u'files')
        docs = files_ref.stream()
        print("[LOG get-files] Query executada.")
        
        file_list = []
        for doc in docs:
            doc_data = doc.to_dict()
            if not doc_data: continue # Ignora documentos vazios ou malformados

            # Converte o timestamp de forma segura
        ...
        # Ordena a lista no Python, que é mais seguro que no banco de dados
        file_list.sort(key=lambda x: x['data']['uploadTimestamp'] or '', reverse=True)
        print(f"[LOG get-files] Processamento concluído. {len(file_list)} arquivos encontrados.")
        return jsonify(file_list)
    except Exception as e:
        print(f"[LOG get-files] ERRO INESPERADO: {str(e)}")
        return jsonify({"error": f"Erro fatal ao buscar lista de arquivos: {str(e)}"}), 500
